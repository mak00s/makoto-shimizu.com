...
サイトのパフォーマンス優先という目的意識を持ってサーバーサイドGTMを使ってみて、いろいろわかりました

* サーバーサイドGTMは手動でプロビジョニングしてGCPのリージョンを東京にする
* iTP対策としてサブドメインは必須
...
* サイトに直接導入するのはGA v1 (Universal Analytics)ではなくGA v2 (App + Web)が良い
  ...
  * サーバーサイドGTMでGA v2のビーコンを分割してGA v1のビーコンに変換できる
  * サーバーサイドGTMでGAのカスタムディメンションもセットできる
* サーバーサイドGTMのトリガーではクライアント名や計測の条件を追加すると良い
  * 複数のサイト用のサブドメインを一つのサーバーサイドGTMに割り当てても動作した
